	...
	class Media < ApplicationRecord
		...
		private
			...
			def set_keywords_and_tags
				...
				new_keywords = "#{self.user} #{self.title} #{self.category}".downcase.split( /\W/ ).delete_if{ |elem| elem.length <= 2 }.delete_if{ |elem| common_terms.include?( elem ) }.uniq
